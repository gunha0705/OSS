...
def normalize_data(n_cases, n_people, scale):
    ...
    for idx, n in enumerate(n_cases):
        norm_cases.append(int(n_cases) / int(n_people) * int(scale))
    return norm_cases
...
for idx, pop in enumerate(n_people):
    ratio = n_people / sum_people * 100 # TODO: The ratio of new cases to the total
...
print('| ------ | --------- | --------- | -------------- |')
for idx, pop in enumerate(n_people):
    ratio = n_covid / sum_people * 100# TODO: The ratio of new cases to the total
    print('| %s | %d | %.1f | %.1f |' % (regions[idx], pop, ratio, norm_covid))
